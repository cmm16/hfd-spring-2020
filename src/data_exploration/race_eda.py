...
def run_race_eda(output_dir, train_df):
    ...
    general_eda.plot_volume_chart(
        output_dir,
        race_avgs,
        "Average Calls per Racially Homogeneous Block Groups",
        "Homogeneous Race",
        [0, 1, 2, 3],
        races,
    )
...
def data_wrangling(train): 
    ...
    sizes = [
        white_blocks.shape[0],
        hisp_blocks.shape[0],
        blk_blocks.shape[0],
        asian_blocks.shape[0],
    ]
    ...
    race_avgs = pd.DataFrame(
        [white_blocks_avg, hisp_blocks_avg, blk_blocks_avg, asian_blocks_avg]
    )
