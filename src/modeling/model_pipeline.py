...
class LGBModelPipeline:
    ...
    def optimize(self):
        ...
        with warnings.catch_warnings():
            ...
            LGB_BO.maximize(init_points=40, n_iter=30, acq='ucb')
    ...
    def lgb_bayesian(self, params):
        formated_params = format_params(params)
        KFolds = KFold(n_splits=5, shuffle=True, random_state=9).split(self.x_train, self.y_train)
        scores = []
        for fold in KFolds:
            lgb_train, lgb_valid, fold_val_x, fold_val_y = self.generate_lgb_fold_data(self, fold)

            num_rounds = 5000
            clf = lgb.train(formated_params, lgb_train, num_rounds, valid_sets=[lgb_valid], verbose_eval=False,
    ...
    def generate_lgb_fold_data(self, fold):
        ...
        lgb_train = lgb.Dataset(fold_train_x, fold_train_y, free_raw_data=False)
        lgb_valid = lgb.Dataset(fold_val_x, fold_val_y, free_raw_data=False)
