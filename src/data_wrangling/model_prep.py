...
def combine_demographic_features(df):
    ...
    new_df["pctAdult"] = (100.0 * df.loc[:, 'TotPop18'] / df.loc[:, 'TotalPop']) - df.loc[:, 'pctAge65p']
    ...
    new_df["raceVar"] = np.var(df[["pctHisp", "pctNHasi", "pctNHblk", "pctNHblk", "pctNHoth", "pctNHwht"]],
                               1)  # race variance
    ...
    new_df["eduVar"] = np.var(new_df[["pct_ltHS", "pct_HS", "pct_Coll", "pct_grad"]], 1)  # education variance
    ...
    # Employment
    new_df["pctDiffMFEmp"] = df["pct_Prof_M"] - df["pct_Prof_F"]  # Percent difference of male female employment
    ...
    new_df["estCommTime"] = (df.pctComm00_14 * 7) + (df.pctComm15_29 * 22) + (df.pctComm30_59 * 44.5) + (
                df.pctComm60p * 70)
...
def table_joins(initial_df):
    df_acres = pd.read_csv("/home/cole-work/PycharmProjects/hfd-spring-2020/data/acres.csv")
    ...
    intersections_df = intersections_df.groupby("BlockGroup").count().reset_index()[["BlockGroup", "OBJECTID"]].rename(
        {"OBJECTID": "Intersections"}, axis=1)
    df = initial_df.merge(df_acres[["Block_Group", "Acres"]], how="inner", left_on="Block_Group",
                          right_on="Block_Group")
    df = df.merge(df_fire_station[["Name", "FIPS_Poly_Cen_x", "FIPS_Poly_Cen_y", "Near_Station_Number"]], how="inner",
                  left_on="Block_Group", right_on="Name").drop(["Name"], axis=1)
    df = df.rename({"FIPS_Poly_Cen_x": "Center_X", "FIPS_Poly_Cen_y": "Center_Y"}, axis=1)
    df = df.merge(intersections_df, how="left", left_on="Block_Group", right_on="BlockGroup").drop(["BlockGroup"],
                                                                                                   axis=1)
...
def split_tragets(df, intial_df):
    target_df = intial_df[
        ["Block_Group", "fire", "health", "injuries_external", "mental_illness", "motor", "other"]].set_index("Block_Group")
    ...
    target_df = target_df.rename({
        ...
        "per_other": "Percent Other Calls"}, axis=1)

    target_df = target_df.drop(["fire", "health", "injuries_external", "mental_illness", "motor", "other", "total_calls"], axis=1)
...
def model_prep(initial_df):
    ...
    df = remove_nulls(df)
    target_df = split_tragets(df, initial_df)

    dem_only_preds = df.drop(["Acres", "Near_Station_Number", "Center_X", "Center_Y", "Intersections"], axis=1)
