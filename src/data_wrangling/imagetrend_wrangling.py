...
def parsetime(data):
    ...
    data[["parse", "timestamp"]] = pd.DataFrame(
        data.parse.values.tolist(), index=data.index
    )
    data["parse"] = data["parse"].str.split("-")
    data[["Year", "Month", "Date"]] = pd.DataFrame(
        data.parse.values.tolist(), index=data.index
    )
...
def wrangle_image_trend(base_dir, save_dir, arg):
    ...
    df3 = pd.read_csv(
        join(
            base_dir,
            "Jan 1 2020 thru Apr 12 2020 - Event_Data_For_Rice_University (Rpt_Data_Rice_Events).csv",
        )
    )
    df3 = df3.rename(
        {
